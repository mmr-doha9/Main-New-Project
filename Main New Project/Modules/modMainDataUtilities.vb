Public Module modMainDataUtilities
    ...
    '        frmSearchTran.DataGridView.DataSource = ds
    '        frmSearchTran.DataGridView.DataMember = strGTableName
    '        frmSearchTran.DataGridView.AutoResizeColumns()

    '        frmSearchTran.ComboBox.Items.Clear()
    '        For Each dc As DataColumn In ds.Tables.Item(strGTableName).Columns
    '            If (dc.ColumnName) = "ID" Then
    '                frmSearchTran.DataGridView.Columns(0).Visible = False
    '            ElseIf (dc.ColumnName) = "CompanyID" Then
    '                frmSearchTran.DataGridView.Columns(frmSearchTran.DataGridView.Columns.Count - 1).Visible = False
    '            Else
    '                Dim strTmp As String = FGetSearchName(dc.ColumnName, 0)
    '                If strTmp = "" Then
    '                    frmSearchTran.DataGridView.Columns(dc.ColumnName).Visible = False
    '                Else
    '                    frmSearchTran.ComboBox.Items.Add(strTmp)
    ...
    '        Next
    '    End If
    '    frmSearchTran.Tag = strGTableName
    '    frmSearchTran.Text = StrFrmText
    '    If intComboIndex > 0 Then frmSearchTran.ComboBox.SelectedIndex = intComboIndex Else frmSearchTran.ComboBox.SelectedIndex = 0
    '    frmSearchTran.txtBox.Focus()
    '    frmSearchTran.ShowDialog()
    '    If strSearchGridVal = Nothing Then Return "0" Else Return strSearchGridVal
    '    frmSearchTran.Tag = "" : strSearchGridVal = ""
    'End Function
    ...
    Public Function FSearch(ByVal strGTableName As String, Optional ByVal strGKeyField As String = "", Optional ByVal strCondition As String = "", Optional ByVal StrFrmText As String = "", Optional ByVal strSearchBeginName As String = "", Optional ByVal strOrderBy As String = "", Optional ByVal intComboIndex As Integer = 0, Optional ByVal ReturnIntComboIndex As Integer = 1) As String
        ...
        frmSearchTran.Tag = strGTableName
        frmSearchTran.Text = StrFrmText
        frmSearchTran.TxtTmp.Text = ReturnIntComboIndex
        If intComboIndex > 0 Then frmSearchTran.ComboBox.SelectedIndex = intComboIndex Else frmSearchTran.ComboBox.SelectedIndex = 0
        frmSearchTran.ShowDialog()
        If strSearchGridVal = Nothing Then Return "0" Else Return strSearchGridVal
        frmSearchTran.Tag = "" : strSearchGridVal = ""
    ...
    Public Function FTreeSearch(ByVal TreeBranches As TreeBranches, ByVal strTableName As String, Optional ByVal strKeyField As String = "", Optional ByVal strKeyValue As String = "", Optional ByVal strCondition As String = "", Optional ByVal strFrmTitle As String = "بداية البحث", Optional ByVal bolCompanyID As Boolean = True, Optional ByVal bolUserId As Boolean = True, Optional ByVal strOrderBy As String = "", Optional ByVal strSecondField As String = "Name", Optional ByVal ReturnType As SearchReturnType = SearchReturnType.String) As String
        ...
        Select Case TreeBranches
            ...
            Case TreeBranches.SingleBranch
                frmTreeSearch2.LoadDataSet(strTableName, strKeyField, strFrmTitle, bolCompanyID, bolUserId, strOrderBy, strSecondField, ReturnType, strCondition)
        ...
        Try
            ...
            If DataType = DataConnection.SqlServer Then
                ...
                If TmpTmpdr.Read() = True Then
                    strTmp = TmpTmpdr.Item(strFldName)
                Else
                    strTmp = ""
                End If
                TmpTmpdr.Close() : Call TmpCON.Close()
                Return strTmp
    ...
    Public Function FGetFeildValues(ByVal strTableName As String, ByVal strFldName As String, ByVal strCondition As String, CONString As SqlClient.SqlConnection, Optional ByVal bolCompId As Boolean = False, Optional ByVal bolUserID As Boolean = False, Optional ByVal strOrderBy As String = "") As String
        Try
            Dim sql As String = ""
            If strCondition = "" Then
                sql = "SELECT " & "[" & strFldName & "]" & "   From " & strTableName
            ...
            If DataType = DataConnection.SqlServer Then
                'Dim TmpCON As New SqlClient.SqlConnection(Db_Conn)
                'TmpCON.Open()
                Dim cmd As New SqlClient.SqlCommand(sql, CONString)
                Dim TmpTmpdr As SqlClient.SqlDataReader = cmd.ExecuteReader
                Dim strTmp As String
                ...
                Else
                    strTmp = ""
                End If
                TmpTmpdr.Close() ': Call CONString.Close()
                Return strTmp
    ...
    Public Function FGetFeildValuesArry(ByVal strTableName As String, ByVal strFldName As String, ByVal strCondition As String, Optional ByVal bolCompId As Boolean = False, Optional ByVal bolUserId As String = "", Optional ByVal strOrderBy As String = "") As String()
        Try
            ...
            If DataType = DataConnection.SqlServer Then
                ...
                Loop
                TmpTmpdr.Close() : TmpCON.Close() : cmd.Dispose()
            ElseIf DataType = DataConnection.Access Then
                Dim TmpCON As New OleDb.OleDbConnection(Db_Conn)
                TmpCON.Open()
                Dim cmd As New OleDb.OleDbCommand(sql, TmpCON)
        ...
        Try
            ...
            If DataType = DataConnection.SqlServer Then
                ...
                If dr.Read() = True Then
                    dr.Close() : TmpCON.Close() : Cmd.Dispose()
                    Return True
                Else
                    dr.Close() : TmpCON.Close() : Cmd.Dispose()
                    Return False
    ...
    Public Function FFormatDateTimeFormat(ByVal DateVal As Date) As String
        If DataType = DataConnection.SqlServer Then
            'Return Format(DateVal, "yyyy-MM-dd hh:mm:ss tt")
            Return Format(DateVal, "yyyy-MM-dd hh:mm:ss tt")
        ElseIf DataType = DataConnection.Access Then
            'Return FormatDateTime(DateVal, DateFormat.GeneralDate)
        End If
    ...
    Public Function FFormatTime(ByVal DateVal As Date) As DateTime
        If DataType = DataConnection.SqlServer Then
            Return Format(DateVal, "hh:mm tt")
        ElseIf DataType = DataConnection.Access Then
            'Return FormatDateTime(DateVal, DateFormat.GeneralDate)
    ...
    Public Function FAutoCompleteItem(ByVal strTableName As String, ByVal strFldCode As String, ByVal strFldName As String, Optional ByVal strCondition As String = "", Optional ByVal bolCompId As Boolean = True, Optional ByVal strOrderBy As String = "") As List(Of String)
        ...
        If DataType = DataConnection.SqlServer Then
            ...
            For Each r In dt.Rows
                lstString.Add(r.Item(strFldName).ToString)
            Next
            Dp.Dispose()
        ElseIf DataType = DataConnection.Access Then
            Dim Dp As OleDb.OleDbDataAdapter = New OleDb.OleDbDataAdapter(strSQL, CONAccess)
    ...
    Public Sub SFillCombo(ByVal ctrl As ComboBox, ByVal strTableName As String, ByVal strFldCode As String, ByVal strFldName As String, Optional ByVal strCondition As String = "", Optional ByVal bolCompId As Boolean = True, Optional ByVal strOrderBy As String = "", Optional ByVal SelectItem As ComboLoadItemSelect = ComboLoadItemSelect.Min)
        ...
        ElseIf SelectItem = ComboLoadItemSelect.Nothing Then
            If ctrl.Items.Count <> 0 Then ctrl.SelectedValue = 0
        End If
    End Sub
    ...
    Public Function FGetMaxNum(ByVal strTblName As String, ByVal strFldName As String, Optional ByVal IDCom As Boolean = False, Optional ByVal IDUser As Boolean = False) As String
        Return FGetColumnValue(strTblName, strFldName, Expresion.Max)
    End Function
    ...
    Public Function FMax(ByVal strField As String, ByVal strTable As String, Optional ByVal strCondition As String = "", Optional ByVal IDCom As Boolean = False, Optional ByVal IDUser As Boolean = False) As Object
        ...
        If IDUser = True Then
            sqlTmp = sqlTmp & " Where UserID='" & strUserID & "'"
        End If
        If strCondition <> "" Then
    ...
    Public Function FAddNewRecord(ByVal TabelName As String, ByVal Columns As String, ByVal Values As String, ByVal bolCompId As Boolean) As Boolean
        Try
    ...
    Public Function FAddNewRecord(ByVal TabelName As String, ByVal Columns As String(), ByVal Values As String()) As Boolean
        Try
    ...
    Public Function FAddNewRecord(ByVal TabelName As String, ByVal Columns As List(Of String), ByVal Values As List(Of String), Optional ByVal bolCompId As Boolean = False) As Boolean
        Try
            Dim sql As String = "insert into " & TabelName & "(" & FGetArraySingle(Columns) & " values( " & FGetArray(Values)
            If bolCompId = True Then
                sql = sql & " WHERE CompanyID='" & strCompanyID & "'" & ";"
    ...
    Public Function FAddNewRecord2(ByVal TabelName As String, ByVal Columns As List(Of String), ByVal Values As List(Of String), Optional strIdColName As String = "ID") As Integer
        Try
            If DataType = DataConnection.SqlServer Then
                Dim sql As String = "insert into " & TabelName & "(" & FGetArraySingle(Columns) & " Output inserted." & strIdColName & " values( " & FGetArray(Values)
                Dim cmd As New SqlClient.SqlCommand(sql, CON)
                Dim tmp As VariantType = cmd.ExecuteScalar()
                cmd.Dispose()
                Return tmp
            Else
                Dim sql As String = "insert into " & TabelName & "(" & FGetArraySingle(Columns) & " values( " & FGetArray(Values)
    ...
    Public Function FAddNewRecord(ByVal TabelName As String, ByVal ParamArray Values As String()) As Boolean
        Try
    ...
    Public Function FEditeRecord(ByVal Tabelname As String, ByVal strCondition As String, CONString As SqlClient.SqlConnection, ByVal Columns As String(), ByVal ParamArray Values As String()) As Boolean
        Try
            Dim sql As String = "update " & Tabelname & FSetColumnArray(Columns, Values) & strCondition
            If DataType = DataConnection.SqlServer Then
                Dim cmd As New SqlClient.SqlCommand(sql, CONString)
                cmd.ExecuteNonQuery()
                cmd.Dispose()
    ...
    Public Function FGetColumnValue(ByVal Tabelname As String, ByVal ColumnName As String, ByVal Expresion As Expresion, Optional ByVal strCondition As String = "") As Object
        Try
            ...
            End Select
            If strCondition <> "" Then strsqlStatement = strsqlStatement & " Where " & strCondition
    ...
    Public Function FDeleteRecord(ByVal Tabelname As String, ByVal ColumnName As String, ByVal value As String) As Integer
        Try
    ...
    Public Function FDeleteRecord(ByVal Tabelname As String, ByVal strCondition As String) As Integer
        Try
    ...
    Public Function FDeleteRecord(ByVal Tabelname As String, Optional ByVal bolReset As Boolean = False) As Integer
        Try
            ...
            If DataType = DataConnection.SqlServer Then
                ...
                If bolReset = True Then
                    cmd.CommandText = "TRUNCATE TABLE [" & Tabelname & "];"
                    Return cmd.ExecuteNonQuery()
                End If
            ElseIf DataType = DataConnection.Access Then
                Dim cmd As New OleDb.OleDbCommand(sql, CONAccess)
    ...
    Public Function FCheckDelete(ByVal intCode As String, strFldName As String, Optional lstTable As List(Of String) = Nothing) As Boolean
        Try
            If IsNothing(lstTable) Then
                If FGetFeildValues("Tblt_EnergyThefts", "ID", strFldName & "=" & intCode) <> Nothing Then Return False
                If FGetFeildValues("Tblt_EnergyTheftsSecond", "ID", strFldName & "=" & intCode) <> Nothing Then Return False
                ''
                If FGetFeildValues("Tblt_Disagreed", "ID", strFldName & "=" & intCode) <> Nothing Then Return False
                If FGetFeildValues("Tblt_DisagreedSecond", "ID", strFldName & "=" & intCode) <> Nothing Then Return False
                ''
                If FGetFeildValues("Tblt_Zaina", "ID", strFldName & "=" & intCode) <> Nothing Then Return False
                If FGetFeildValues("Tblt_ZainaSecond", "ID", strFldName & "=" & intCode) <> Nothing Then Return False
            Else
                For i As Integer = 0 To lstTable.Count - 1
                    If FGetFeildValues(lstTable(i).ToString, "ID", strFldName & "=" & intCode) <> Nothing Then Return False
                Next
            End If
    ...
    Public Function FCopyRecord(strMainTbleFrom As String, strConditions As String, strMainTbleTo As String) As Boolean
        Try
            Dim lstFieldName, lstValue As New List(Of String)
            Dim strSqlTmp As String = "select * from " & strMainTbleFrom & " Where " & strConditions

            Dim cmd As New SqlClient.SqlCommand(strSqlTmp, CON)
            Dim dr As SqlClient.SqlDataReader = cmd.ExecuteReader
            Do While dr.Read
                For i As Integer = 0 To dr.FieldCount
                    lstValue.Add(dr.Item(i))
