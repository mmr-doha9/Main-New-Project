Public Class frmEnergyDevices
    Implements IFormMainFunction

    Dim strTable As String = "Tblc_EnergyDevices"
    ...
    Private Sub frmMabahsCode_Load(sender As Object, e As EventArgs) Handles Me.Load
    ...
    Public Sub SAddNew() Implements IFormMainFunction.SAddNew
        ...
        TxtBox2.Focus()
        Call SFillCombo(cmbType, "Tblc_KiloHorseWat", "Code", "Name", "", False, "Code", ComboLoadItemSelect.Min)
    ...
    Public Sub SDelete() Implements IFormMainFunction.SDelete
        If MsgBox("هل تريد حذف السجل الحالي", CType(MsgBoxStyle.OkCancel + MsgBoxStyle.Critical + MsgBoxStyle.MsgBoxRtlReading + MsgBoxStyle.MsgBoxRight, Global.Microsoft.VisualBasic.MsgBoxStyle), ProgName) = MsgBoxResult.Ok Then
            Dim lstTable As New List(Of String) : lstTable.AddRange({"Tblt_EnergyTheftsDevices", "Tblt_EnergyTheftsSecondDevices"})

            If FCheckDelete(Val(TxtBox1.Text), "EnergyDevicesCode", lstTable) = True Then
    ...
    Public Sub SOpen() Implements IFormMainFunction.SOpen
        ...
        If DataType = DataConnection.SqlServer Then
            ...
            If dr.Read() = True Then
                ...
                cmbType.SelectedValue = dr.Item(FGetTagData(cmbType, Valadation.Tag))
    ...
    Private Sub SFillCtl(ByVal ctlName As Control)
        ...
        With lstCtlTag
            ...
            .Add("[" & FGetTagData(cmbType, Valadation.Tag) & "]")
